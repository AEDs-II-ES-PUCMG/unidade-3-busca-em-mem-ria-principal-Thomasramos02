...
public class App {
    ...
    */
     ...
     */
    static AVL<Integer, Cliente> lerClientes(String nomeArquivoDados, Function<Cliente, Integer> extratorDeChave) {
        ...
        try {
            arquivo = new Scanner(new File(nomeArquivoDados), Charset.forName("UTF-8"));
            numCliente = Integer.parseInt(arquivo.nextLine());
            
    		for (int i = 0; i < numCliente; i++) {
    			linha = arquivo.nextLine();
    			cliente = Cliente.criarDoTexto(linha);
                Integer chave = extratorDeChave.apply(cliente);
                clientesCadastrados.inserir(chave, cliente);
    		}
            quantosClientes = numCliente;
        } catch (IOException excecaoArquivo) {
    ...
    static <K> Produto localizarProduto(ABB<K, Produto> produtosCadastrados, K procurado) {
        ...
        static <K> Pedido localizarClienteID(ABB<K, Cliente> ClienteCadastrados, int idCliente2) {
    
    
        idCliente2 = lerOpcao("Digite o identificador do cliente desejado: ", Integer.class);
        
        return localizarClienteID(ClienteCadastrados, idCliente2);
    }
    ...
    private static Lista<Pedido> gerarPedidos(int quantidade) {
        ...
        for (int i = 0; i < quantidade; i++) {
        	...
        	idCliente = -1;
        	cliente = null;
            quantosClientes = sorteio.nextInt(quantosClientes) + 10_000;
            ...
            // chamando inserirNaTabelaPedidosDoCliente(cliente, pedido).
             for(int t = 0; t < quantosClientes; t++){
     ...
     * @throws IOException 
     */
    public static void pedidosDoCliente() throws IOException {
        ...
        Pedido pedidosDoCliente;
        int idCliente = lerOpcao("Digite o identificador do produto desejado: ", Integer.class);
        Pedido cliente = localizarClienteID(null, idCliente);
        pedidosDoCliente = pedidosDoCliente.pesquisar(idcliente);
        String nomeArquivo = "RelatorioProduto" + cliente.hashCode() + ".txt";  
        FileWriter arquivoRelatorio = new FileWriter(nomeArquivo, Charset.forName("UTF-8"));
    ...
    public static void main(String[] args) throws IOException {
        ...
        Function<Cliente, Integer> extratorDeChave = null; // usado apenas para iniciar o ler cliente no menu
        ...
        clientesPorId = lerClientes(nomeArquivoDados, extratorDeChave);
